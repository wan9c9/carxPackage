...
#' @param trueX the true parameter (coefficients) for exogenous variable.
#' @param trueAR the true AR parameter.
#' @param trueSigma the true sigma parameter.
...
#' @param alpha the alpha used to construct the confidence interval for estimated parameters
#' @param nLag the number of lags to construct the Ljung-Box test statistic, see \code{\link{goodnessOfFit.carx}}
...
#' \dontrun{simulation.carx()} #using default settings
#' \dontrun{simulation.carx(ucl=Inf)} # no right censoring
#' \dontrun{simulation.carx(lcl=-Inf)} # no left censoring

simulation.carx <-function(trueX=c(0.2,0.4),
			   ...
			   fullEstimation=FALSE)
{
	...
	nAR <- length(trueAR)
	truePrmtr <- c(trueX,trueAR,trueSigma)
	...
	iter <- 1:nRep
	rslt <- parallel::mclapply(iter,simEst,mc.cores=parallel::detectCores())
	...
	xnames <- paste0('X',1:length(trueX))
