...
fitted.carx <- function(object,...)
{
  ...
  {
    message("Cached 'fitted' found in supplied object, the cached fitted values are returned. To force re-computation, set object$fitted=NULL before calling this function.")
    invisible(object$fitted)
  }else{
    nObs  <- object$nObs
    p  <- object$p
