...
simulation2.carx <-function(trueX=c(0.2,0.4), 
													 trueAR=c(-0.28,0.25), 
													 trueSigma=0.60,
													 lcl=-1,
													 ucl=Inf,
													 ...
													 debug=F)
{
  ...
  trueAR1 <- numeric(nAR+1)
	...
	#find censorLimit
	dat <- carx.simulate(10000, trueAR1, trueX, trueSigma, -1, 1, seed=0)
	...
	simEst <-function(iRep)
	{
		...
		for(n in nObs)
		{
			...
			{ 
				...
				dat <- carx.simulate(n, trueAR1, trueX, trueSigma, lcl, uppercl, seed=37513*iRep)
				if(i == 1)
				{
